...
describe(filename(__filename), function() {
    ...
    it(itname('[.json]', 'Lấy json của một sách'), async function() {
    ...
    it(itname('[.getJsonBooks(realm)]', 'Lấy json của tất cả sách'), async function() {
        const realm = await database();
    ...
    it(
        itname('[.has(realm, instance)]', 'Tồn tại đối tượng trong Class'),
        async function() {
            const realm = await database();
